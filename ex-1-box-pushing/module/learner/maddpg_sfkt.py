...
class maddpg():
    def __init__(self, args, mas_label):
        ...
        # self.TD_error = tf.einsum('ij,jk->ik', self.sfs_target - self.SFs, self.R_weight)
        # self.loss_c = tf.reduce_mean(tf.square(self.TD_error), name=self.label + "loss_c")
        # self.loss_r = tf.nn.l2_loss(self.reward - self.reward_t, name=self.label + "loss_r")
        ...
        # self.loss_c = [tf.reduce_mean(tf.square(self.TD_error[:, idx_task]), name=self.label + "loss_c_task_"+str(idx_task))
