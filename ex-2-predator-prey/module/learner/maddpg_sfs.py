...
class maddpg():
    def __init__(self, args, mas_label):
        ...
        # Phase I
        self.Q_target = tf.placeholder(tf.float32, [None, 1], name=self.label + "target_sfs")
        ...
        # Phase II
        self.act_t_target = [tf.placeholder(tf.float32, [None, self.d_a], name=self.label + "target_actions_agt_"+str(i))
                             for i in range(self.n_agents)]
        self.act_error = [self.act_t[i] - self.act_t_target[i] for i in range(self.n_agents)]
        self.loss_a_2 = [tf.reduce_mean(tf.reduce_sum(tf.square(self.act_error[i]), axis=1), name=self.label + "loss_agt_"+str(i)) for i in range(self.n_agents)]
        self.trainer_a_2 = [[tf.train.AdamOptimizer(self.lr_a).minimize(self.loss_a_2[i], var_list=self.a_var[i])
                             for i in range(self.n_agents)]]
        ...
        with tf.name_scope("losses"):
            ...
            self.loss_a_sum_2 = [tf.summary.scalar(self.label + "loss_a_sum_agt_" + str(i), self.loss_a_2[i]) for i in range(self.n_agents)]
            ...
            self.merge_a_2 = tf.summary.merge(self.loss_a_sum_2)

        # soft update for target network
        self.A_update = []
        for idx_agt in range(self.n_agents):
            update = [self.a_tar_var[idx_agt][i].assign(
                tf.multiply(self.a_var[idx_agt][i], self.tau) + tf.multiply(self.a_tar_var[idx_agt][i],
                                                                            1 - self.tau))
    ...
    def get_target_sfs(self, features, obs_next, sess):
        sfs_next = self.SFs_t.eval(feed_dict={self.obs_tt[0]: obs_next[:, 0, :],
                                              self.obs_tt[1]: obs_next[:, 1, :]},
                                   session=sess)
        sfs_predict = features + self.gamma * sfs_next
        return sfs_predict

    def get_target_q(self, rewards, obs_next, sess):
        q_next = self.Q_t.eval(feed_dict={self.obs_tt[0]: obs_next[:, 0, :],
                                            self.obs_tt[1]: obs_next[:, 1, :]},
                                 session=sess)
        q_predict = rewards + self.gamma * q_next
        return q_predict
