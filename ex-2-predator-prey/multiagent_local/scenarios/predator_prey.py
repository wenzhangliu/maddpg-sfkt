...
class Scenario(BaseScenario):
    def make_world(self, num_agt=2, target_idx=None):
        ...
        for i, agent in enumerate(world.agents):
            ...
            agent.real_mass = real_mass_predator if agent.is_predator else real_mass_prey
        # add landmarks
    ...
    def agent_reward(self, agent, world):
        ...
        for idx_task in range(self.num_tasks):
            if count_capture == (idx_task + 1):
                rew[idx_task] += self.capture_reward
    ...
    def observation(self, agent, world):
        ...
        for other in world.agents:
            if other.is_predator:
                predator_prey_dist.append(np.linalg.norm(other.state.p_pos - world.agents[-1].state.p_pos))
            if other is agent:
                continue
            ...
            #     other_vel.append(other.state.p_vel)
        return np.concatenate([agent.state.p_vel] + [agent.state.p_pos] + [predator_prey_dist] +
